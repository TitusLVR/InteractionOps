...
Face mode rotates the face plane by the typed angle around the pivot side and slides every face vert along its rail to the intersection with the rotated plane: `proj_along_axis * sin(angle) / (rail . n')`, where `n'` is the rotated plane normal. On rails perpendicular to the face this reduces to `proj * tan(angle)`, and on oblique rails (chamfer strips) the verts still land exactly on the rotated plane — so the typed angle IS the resulting tilt in both cases. Verts that share a projection (one profile cross-section, e.g. the end edge of a chamfer strip) slide as a rigid row along the normalized mean of their rail directions, so diverging rails (the two sides of a chamfered corner) can't twist the face into a bowtie. The axis can be steered with F (toggle between two world-aligned in-plane axes), A (align to the intersection line with a picked face), B (longer side of the face's minimum oriented bounding box), or by clicking one of the four orange axis handles on the widget.
...
- Extrude sub-modal mirrors the rail across the sheared face plane (edge mode: across the sheared edge direction) so the extruded segment forms matched mitred ends. Per-vert delays equal `slide_max - slide` (matching the face-mode plane-intersection slide rule), so the pivot edge starts moving only after the saw-off offset is consumed. Confirm deletes the original (interior) face; cancel keeps it.
