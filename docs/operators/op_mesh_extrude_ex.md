...
Both toggles default to on, reproducing the previous (pre-toggle) behavior exactly. Unticking both turns the operator into a plain native extrude-and-move: no attribute propagation happens at all. The angle slider only has an effect while "Continue Parents" is enabled.
...
- Four classes are registered by this module: `IOPS_OT_extrude_attr_fix`, `IOPS_OT_extrude_attr_fix_post`, `IOPS_OT_mesh_extrude_ex_macro`, and `IOPS_OT_mesh_extrude_ex` (the dispatcher). Only the dispatcher is user-facing for keymap/search purposes — the two attribute-fix operators and the macro operator are internal and should not be bound directly, though scripts call the macro (see Implementation Details). Neither attribute-fix operator carries the `INTERNAL` bl_option: as macro steps, that flag would also hide their properties from the macro's F9 redo panel, which is where their per-operation toggles need to be visible. They stay off any menu regardless (nothing registers a menu entry for them), so the only visible side effect is that they can now be found via operator search (F3) like any other registered operator. No panels, menus, or PropertyGroups beyond the two fix operators' own toggles.
