...
This clipping needs the plain unit-speed wavefront metric to line up with straight-line distance to the boundary, so it only applies when every edge weight is 1.0 (i.e. `use_boundary=True`, since a frozen border edge has weight 0). It also degrades automatically and silently to a plain ngon fill covering the whole front — pre-clipping behaviour — if the interior can't be clipped watertight for any reason (a warning is reported when that happens); the mesh is left completely untouched until a valid plan exists, so a failed clip never leaves a partial or broken result.
