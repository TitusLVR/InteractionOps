                ...
                for file in directory:
                    if mat_name in file and file != f.name:
                        ...
                        if normal_map in file:
                            ...
                            mat.node_tree.links.new(NormalMap.inputs['Color'], NormalImage.outputs['Color'])
                            ...
                            SeparateRGB = mat.node_tree.nodes.new('ShaderNodeSeparateRGB')
                            SeparateRGB.location = (-250,-250)

                            mat.node_tree.links.new(SeparateRGB.inputs['Image'], MaskImage.outputs['Color'])
                            mat.node_tree.links.new(bsdf.inputs['Metallic'],SeparateRGB.outputs['R'])
                            mat.node_tree.links.new(bsdf.inputs['Roughness'],SeparateRGB.outputs['G'])
                        else:                            
                            path = os.path.join(dirname, file)
                            texImage = mat.node_tree.nodes.new('ShaderNodeTexImage')
                            texImage.name = file
