...
class IOPS_OT_WidgetRowAdd(bpy.types.Operator):
    bl_idname = "iops.widget_row_add"
    bl_label = "Add Row"
    bl_description = "Add a row to the selected widget"
    bl_options = {"REGISTER"}

    type: EnumProperty(name="Type", items=widget_composer._ROW_TYPE_ITEMS)
    ...
    def execute(self, context):
        ...
        row.type = self.type
        if self.type == "BUTTON":
            row.op = "iops.executor"
        item.rows_index = len(item.rows) - 1
        widget_composer.autosave_selected()
        return {"FINISHED"}


class IOPS_OT_WidgetRowRemove(bpy.types.Operator):
    bl_idname = "iops.widget_row_remove"
    bl_label = "Remove Row"
    ...
    def poll(cls, context):
        item = widget_composer.selected_item()
        return (item is not None and not item.builtin
                and 0 <= item.rows_index < len(item.rows))

    def execute(self, context):
        item = widget_composer.selected_item()
        item.rows.remove(item.rows_index)
        item.rows_index = min(item.rows_index, len(item.rows) - 1)
        widget_composer.autosave_selected()
        return {"FINISHED"}


class IOPS_OT_WidgetRowMove(bpy.types.Operator):
    bl_idname = "iops.widget_row_move"
    bl_label = "Move Row"
    bl_description = "Move the selected row up/down"
    bl_options = {"REGISTER"}
    ...
    def execute(self, context):
        ...
        if not (0 <= new < len(item.rows)):
            return {"CANCELLED"}
        item.rows.move(idx, new)
        item.rows_index = new
        widget_composer.autosave_selected()
        return {"FINISHED"}
...
classes = (
    ...
    IOPS_OT_WidgetRowAdd,
    IOPS_OT_WidgetRowRemove,
    IOPS_OT_WidgetRowMove,
