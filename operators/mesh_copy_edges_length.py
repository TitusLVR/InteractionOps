...
class IOPS_MESH_OT_CopyEdgesLength(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if selected_edges:
            ...
            total_length = sum(e.calc_length() for e in selected_edges) / get_unit_scale()
            ...
            bpy.context.window_manager.clipboard = str((selected_verts[0].co - selected_verts[1].co).length)
            self.report({'INFO'}, "Copied the distance between the 2 selected verts to clipboard")
            bm.free()            
            return {"FINISHED"}
        else:
            #Invalid Selection
