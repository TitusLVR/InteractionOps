...
class AlignObjectToFace2(bpy.types.Operator):
    """ Align object to selected face """
    bl_idname = "iops.align_object_to_face2"
    bl_label = "MESH: Align object to face2"
    ...
    def align_to_face(self, idx, axis, flip):
        ...
        # Assemble new matrix
        if axis == 'Z':
            mx_new = Matrix((c, t, n)).transposed().inverted().to_4x4()
        elif axis == 'Y':
            mx_new = Matrix((t, n, c)).transposed().inverted().to_4x4()
        elif axis == 'X':
            mx_new = Matrix((n, c, t)).transposed().inverted().to_4x4()
    ...
    def modal(self, context, event):
        ...
        # ---------------------------------------------------------
                ...
                print("W_XYZ",self.edge_idx)

        elif event.type == "WHEELDOWNMOUSE":
                if self.counter >= 1:
                    self.counter -= 1
                print("W_DWN",self.edge_idx)
                self.align_update(event)

        elif event.type == "WHEELUPMOUSE":
                self.counter += 1
                self.align_update(event)
                print("W_UP",self.edge_idx)
