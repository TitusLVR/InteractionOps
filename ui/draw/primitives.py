...
def polyline(coords: Sequence, *, role: Role, width: str | None = None,
    ...
    w = th.width(width) if width is not None else th.line_width_for(role)
    shader.bind()
...
def edges_3d(coord_pairs: Sequence, *, role: Role, width: str | None = None,
             theme: Theme | None = None, context=None) -> None:
...
def points(coords: Sequence, *, role: Role, size: str | None = None,
    ...
    px = th.point_size(size) if size is not None else th.point_size_for(role)
    shader.bind()
