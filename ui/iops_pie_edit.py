...
class IOPS_MT_Pie_Edit(Menu):
    ...
    def draw(self, context):
        ...
        if context.area.type == "VIEW_3D":
            # 4 - LEFT
            pie.operator("iops.function_1", text="Vertex", icon="VERTEXSEL")
            # 6 - RIGHT
            pie.operator("iops.function_3", text="Face", icon="FACESEL")
            # 2 - BOTTOM
            pie.operator("iops.function_esc", text="Esc", icon="EVENT_ESC")
            # 8 - TOP
            pie.operator("iops.function_2", text="Edge", icon="EDGESEL")
            # 7 - TOP - LEFT
            # pie.separator()
            # 9 - TOP - RIGHT
            # pie.separator()
        ...
        if context.area.type == "IMAGE_EDITOR":
            if context.tool_settings.use_uv_select_sync == True:
                ...
                pie.separator()
                # 9 - TOP - RIGHT
                pie.operator("iops.function_4", text="Island", icon="UV_ISLANDSEL")
